		...
		<c:forEach var="j" begin="0" end="6">
			...
			<c:if test="${((j>=yoil and i==1) or i>1) and chong >= day }">
				<td>${day}<br>	<!-- (첫째주에서 금욜이후거나) 둘째주이고 31일전까지는 day 출력 -->
					...
					%>
					<c:set var="dday" value="<%=new java.util.Date(year-1900, month-1, day)%>"/>
					...
					<c:if test="${dday >= today}"> <!-- 달력에 출력되는 날짜가 오늘 이후인 경우는 true -->
						...
						<!-- room_state에 필요한 rid 받아오기 -->
						<c:set var="rid" value="${hdto.id}}"/>
						<%
							String rid = pageContext.getAttribute("rid").toString();
							String xday = pageContext.getAttribute("dday").toString();
							int chk = hdao.room_state(rid, xday); 
							pageContext.setAttribute("chk", chk);
						%> 
						<c:if test="${chk == 0 }">
							<a style="color:blure;text-decoration:none" href="reserve_next.jsp?year=${year}&month=${month}&day=${day}&room_id=${hdto.id}"> ${hdto.name} </a> <br>
						</c:if>
						<c:if test="${chk != 0 }">
							<span style="color:red">${hdto.name}</span><br>
						</c:if>
